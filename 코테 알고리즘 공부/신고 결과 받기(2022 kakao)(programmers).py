...
def solution_1(id_list, report, k):
    rep = [[] for i in range(len(id_list))]
    for i in report:
        a = i.split(" ")
        if a[1] not in rep[id_list.index(a[0])]:
            rep[id_list.index(a[0])].append(a[1])
...
print(solution_2(id_list, report, k))
print(solution_3(id_list, report, k))
