...
def solution(order):
    answer = 0
    tac = [i for i in range(len(order), 0, -1)]
    order = list(reversed(order))
    stack = []
    while True:
        if not order:
            break
        if tac and order[-1] != tac[-1]:
            if not stack:
                stack.append(tac.pop())
            elif stack and stack[-1] != order[-1]:
                stack.append(tac.pop())
            elif stack and stack[-1] == order[-1]:
                answer += 1
                order.pop()
                stack.pop()
        elif tac and order[-1] == tac[-1]:
            answer += 1
            tac.pop()
            order.pop()
        elif stack and order[-1] == stack[-1]:
            answer += 1
            stack.pop()
            order.pop()
        elif not tac and stack[-1] != order[-1]:
            break
    return answer
