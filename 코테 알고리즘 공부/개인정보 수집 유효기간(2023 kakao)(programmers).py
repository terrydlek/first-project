...
def solution_1(today, terms, privacies):
    ...
    for i in terms:
        term.append(i.split())
    tem = {i[0]: int(i[1]) for i in term}
    privacy = []
    for i in privacies:
        privacy.append(i.split())
    clas = []
    for i in range(len(privacy)):
        ...
        if minus[2] == 0:
            ...
            minus[1] = int(a[1])
            minus[0] = int(a[0])
        oneminus.append(minus)
        clas.append(privacy[i][1])
    for i in range(len(clas)):
        if oneminus[i][1] + tem[clas[i]] > 12:
            year = oneminus[i][0] + (oneminus[i][1] + tem[clas[i]] - 1) // 12
            if (oneminus[i][1] + tem[clas[i]]) % 12 == 0:
                month = 12
            else:
                month = (oneminus[i][1] + tem[clas[i]]) % 12
            day = oneminus[i][2]
        else:
            year = oneminus[i][0]
            month = oneminus[i][1] + tem[clas[i]]
            day = oneminus[i][2]
        if year < int(to[0]):
            answer.append(i + 1)
        elif year == int(to[0]):
