...
# 주식 거래 기록 추가
def insert_transaction(symbol, date, price, quantity, transaction_type):
    cursor.execute('''
        INSERT INTO transactions (symbol, date, price, quantity, transaction_type)
        VALUES (?, ?, ?, ?, ?)
    ''', (symbol, date, price, quantity, transaction_type))
    conn.commit()
...
# 주식 구매 함수
def buy_stock(symbol, date, price, quantity):
    insert_transaction(symbol, date, price, quantity, 'BUY')

# 주식 판매 함수
def sell_stock(symbol, date, price, quantity):
    insert_transaction(symbol, date, price, quantity, 'SELL')

# 주식 손익 계산 함수
def calculate_profit_loss(symbol):
    cursor.execute(f"SELECT * FROM transactions WHERE symbol = ? ORDER BY date", (symbol,))
    ...
    for transaction in transactions:
        if transaction[5] == 'BUY':
            total_investment += transaction[3] * transaction[4]
        elif transaction[5] == 'SELL':
            total_sold_value += transaction[3] * transaction[4]

    cursor.execute(f"SELECT * FROM stocks WHERE symbol = ? ORDER BY date DESC LIMIT 1", (symbol,))
    stock_data = cursor.fetchone()
    if stock_data:
        current_price = stock_data[6]  # 'close' column
        current_quantity = stock_data[7]  # 'volume' column
        total_investment += current_price * current_quantity

    total_profit_loss = total_sold_value - total_investment
    return total_profit_loss

# 포트폴리오 추적 기능 (수정하여 손익 계산 정확하게 처리)
def track_portfolio(portfolio):
    total_profit_loss = 0
    for symbol, quantity in portfolio.items():
        profit_loss = calculate_profit_loss(symbol)
        total_profit_loss += profit_loss

    return f"Total Profit/Loss for the portfolio: {total_profit_loss}"
...
# 주식 구매 예시
buy_stock('AAPL', '2023-07-20', 140.50, 10)
buy_stock('GOOG', '2023-07-21', 2785.30, 5)

# 주식 판매 예시
sell_stock('AAPL', '2023-07-28', 145.20, 5)
